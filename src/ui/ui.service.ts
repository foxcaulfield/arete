...
export class UiService {
	...
	public async getDashboard(
		...
		if (viewMode === "mine") {
			...
			const myCollectionIds = myCollections.map((c): string => c.id);
			...
			const totalAttempts = attemptsData.reduce((sum, a): number => sum + a._count.id, 0);
			const correctAttempts = attemptsData.find((a): boolean => a.isCorrect)?._count.id || 0;
			...
			const coverage =
				myExercisesCount > 0 ? ((uniqueExercisesAttempted.length / myExercisesCount) * 100).toFixed(1) : "0";
			...
			const uniqueCollectionsPracticed = new Set(collectionsAttempted.map((a): string => a.exercise.collectionId))
				.size;
	...
	public async getProfile(session: UserSession): Promise<object> {
		const userId = session.user.id;
		...
		// Get user stats
		const [collectionsCount, exercisesCount, attemptsData, uniqueExercisesAttempted] = await Promise.all([
			...
			this.prismaService.attempt.groupBy({
				by: ["exerciseId"],
				where: { userId },
			}),
		]);
		...
		const coverage = exercisesCount > 0 ? Math.round((uniqueExercisesAttempted.length / exercisesCount) * 100) : 0;
