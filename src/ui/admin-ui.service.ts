...
export class AdminUiService {
  ...
  async getAdminDashboard(_session: UserSession): Promise<object> {
  ...
  async getAdminCollections(_session: UserSession, page: string = "1", limit: string = "20", search?: string): Promise<object> {
  ...
  async getAdminUsers(_session: UserSession, page: string = "1", limit: string = "20", search?: string): Promise<object> {
    const pageNum = parseInt(page) || 1;
    const limitNum = parseInt(limit) || 20;
    const skip = (pageNum - 1) * limitNum;
    const where = search
  ...
  async getAdminUserCreate(): Promise<object> {
    return {
      title: "Create User - Admin",
    };
  }

  async getAdminUserDetail(userId: string): Promise<object> {
  ...
  async getAdminUserEdit(userId: string): Promise<object> {
    const user = await this.prismaService.user.findUnique({
      where: { id: userId },
    });
  ...
  async getAdminExercises(
    ...
    if (filter === "orphans") {
      ...
      const orphanExercises = allExercises.filter((e) => !existingCollectionIds.has(e.collectionId));
      ...
      const exercises = paginatedOrphans.map((e) => ({
        ...e,
        collection: null,
    ...
    const existingIds = new Set(existingCollectionIds.map((c) => c.id));
    const allExerciseCollectionIds = await this.prismaService.exercise.findMany({ select: { collectionId: true } });
    const orphanCount = allExerciseCollectionIds.filter((e) => !existingIds.has(e.collectionId)).length;
  ...
  async getAdminAttempts(
    ...
    if (filter === "orphans") {
      ...
      const orphanAttempts = allAttempts.filter((a) => !existingExerciseIds.has(a.exerciseId));
      ...
      const attempts = paginatedOrphans.map((a) => ({
        ...a,
        exercise: null,
    ...
    const existingIds = new Set(existingExerciseIds.map((e) => e.id));
    const allAttemptExerciseIds = await this.prismaService.attempt.findMany({ select: { exerciseId: true } });
    const orphanCount = allAttemptExerciseIds.filter((a) => !existingIds.has(a.exerciseId)).length;
